...
impl<T: UsbContext>  TemperStick<T> {
    fn new(dev: Device<T>) -> Self{
        TemperStick{handle:(&dev).open().unwrap()}
    }
    ...
    pub fn init(&mut self) -> Result<(), Error>{
        ...
        self.handle.write_control(0x21, 0x09, 0x0201, 0x00, &[0x01, 0x01], TIMEOUT)?;
        
        
        let init_msgs:[[u8; 8]; 3]=[[0x01, 0x80, 0x33, 0x01, 0x00, 0x00, 0x00, 0x00],
        ...
        for i in 0..3 {
            self.handle.write_control(0x21, 0x09, 0x0200, 0x01, &init_msgs[i], TIMEOUT)?;
            
            match self.handle.read_interrupt(0x82, &mut buf, TIMEOUT) {
                Ok(8) => (),
                Ok(x) => { dbg!("only read {} bytes!", x); return Err(Error::Other) },
                Err(e) => return Err(e)
            }
        }
    ...
    pub fn get_temp(&self) -> Result<f32, Error> {
        ...
        self.handle.write_control(0x21, 0x09, 0x0200, 0x01, &msg, TIMEOUT)?;
